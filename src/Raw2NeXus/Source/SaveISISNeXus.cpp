...
/** Constructor
  * @param inRaw :: The input raw file
  * @param outNexus :: The name of the output Nexus file
  */
SaveISISNexus::SaveISISNexus(const std::string& inRaw, const std::string& outNexus) 
  :inputFilename(inRaw),outputFilename(outNexus)
{
}
...
void SaveISISNexus::runlog()
{
  ...
  if (!fil)
  {
    std::cerr << "Cannot find the ICPstatus file. Skipping runlog" << std::endl;
    return;
  }
  ...
  if (!icpevent_fil)
  {
    std::cerr << "Cannot find the ICPevent file" << std::endl;
    return;
  }
...
void SaveISISNexus::selog()
{
  ...
  for(int i = 0; i < potentialLogFiles.size(); ++i)
  {
    ...
    if (!fil)
    {
      std::cout << "Cannot open log file " + potentialLogFiles[i] << std::endl;
      continue;
    }
